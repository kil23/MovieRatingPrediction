...
//		}
	...
	public static List getRecommendedMovies(String user){
		String query ="truncate `splitresults`";
		int i = DBUtils.executeUpdate(query);
		String query1 ="call splitter((SELECT group_concat(replace (genres,'|',',')  SEPARATOR  ',') FROM `movie_history` where userid  like '"+user+"'),',')";
		i=DBUtils.executeUpdate(query1);
		String query3 = "SELECT split_value as genres,count(*) as genres_count FROM `splitresults` group by split_value order by 2 desc limit 0,5";
		List list =DBUtils.getBeanList(movieModel.class, query3);
		String WhereCondition=" WHERE (";
		for (int j = 0; j < list.size(); j++) {
			movieModel mv  = (movieModel) list.get(j);
			WhereCondition+="genres like '%"+mv.getGenres()+"%' OR ";
//			System.out.println(mv.getGenres()+":"+mv.getGenres_count());
		}
		String query4="SELECT * FROM `movie_history` "+WhereCondition;
		query4 =query4.substring(0,Math.max(30, query4.lastIndexOf("OR")));
		query4+=") AND userid not like '"+user+"'";
		System.out.println("\nQuery: "+query4);
		List moviesList = DBUtils.getBeanList(movieModel.class, query4);
		
		for ( i = 0; i < moviesList.size(); i++) {
			movieModel mv1=new movieModel();
			mv1=(movieModel)moviesList.get(i);
			
			String data="Select 1 from movie_history where title like '"+mv1.getTitle()+"' AND userid like '"+user+"'";
		...
		System.out.println("listSize:"+moviesList.size());
		return moviesList;
	}
...
//				System.out.println(stream);
				...
				for (int j = 0; j < tmp.length; j++) {
					...
					if(tmp[j].contains("Action")){
						hm.put("Action",StringHelper.n2i(hm.get("Action"))+1);
...
//					System.out.println("::"+tmp[j]);
		...
		System.out.println("Action movies:"+hm.get("Action"));
		...
		List movieList = DBUtils.getBeanList(movieModel.class, query1);
		for (int i = 0; i < movieList.size(); i++) {
			movieModel mv1=new movieModel();
			mv1=(movieModel)movieList.get(i);
...
//				list1.add(list.get(i));
	...
	public static void addToHistort(String user,String movie,String poster,String genres,String actors){
		...
		boolean v=DBUtils.dataExists(data);
		if (!v) {
			String query = "insert into movie_history(title,userid,poster,genres)"
					+"values(?,?,?,?)";
			int list = DBUtils.executeUpdate(query, movie,user,poster, genres);
			if (list > 0) {
